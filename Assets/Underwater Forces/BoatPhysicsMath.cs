...
public static class BoatPhysicsMath
{
	...
	public static Vector3 PressureDragForce(TriangleData triangleData)
	{
		...
		if (triangleData.cosTheta > 0f)
		{
			...
			pressureDragForce = -(C_PD1 * velocity + C_PD2 * (velocity * velocity)) * triangleData.area * Mathf.Pow(triangleData.cosTheta, f_P)
				* triangleData.normal;
		}
		else
		{
			...
			pressureDragForce = (C_SD1 * velocity + C_SD2 * (velocity * velocity)) * triangleData.area * Mathf.Pow(Mathf.Abs(triangleData.cosTheta), 
				f_S) * triangleData.normal;
		}
